...
Fortunately one of the powerful features of databases is that you can send a query and get back a preview first. SQL provides many operations that you can do inside the database, like counting rows, adding two columns together, filtering on conditions, and lots more. If you've already memorized all of that, great! Otherwise there's the `{dbplyr}` package to handle a lot of the query construction for you.
...
Notice at the top, it says `table<treatments> [?? x 3]`? We know there are only those 8 rows, but the database could have millions. It's only returning this short preview. If we want to manipulate this query, we can do that without forcing the database to give us everything. Let's look at only the rows that match corn plots.
...
And again, we can see what SQL query is really being sent with `explain()`:

```{r paged.print = F}
explain(number_of_plots)
```
...
Other types of joins you might run into are **full**/**outer**/**cross**, **inner**, **right**, **semi**, and **anti**. The help page at `?dplyr::full_join` is a good resource for explaining them, and there are many online tutorials as well. However, not all the joins are implemented in SQLite, so you may have to `collect()` and do them locally inside R. I'm fairly sure all of them are implemented in `{Postgres}` though, so let's move there and start trying to read the PSA On-Farm database.