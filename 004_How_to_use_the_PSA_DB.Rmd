...
So let's do some local computation on the **`site_list`** object we just stored. First we'll make a static map using the `{ggplot2}` package, which is good for making high quality customizable images. Then we'll make an interactive map with `{leaflet}`, a JavaScript package that embeds maps in documents like these or web applications. 
...
inner_join(
  tbl(con, "site_information"),
  tbl(con, "decomp_biomass"),
  by = "code"     # this forces it to only use the `code` col for matching
)
```
...
Let's try an example with three tables. We'll use the decomp table to estimate moisture content for bags at **`time == 0`** (first collection). Then we'll join it to the in-field biomass table and calculate dry equivalents of the fresh biomass there. Then we'll join it back to the site info table to analyze it.

```{r paged.print = FALSE}

bag_moisture_query <- tbl(con, "decomp_biomass") %>% 
  filter(time == 0) %>% 
  mutate(m_factor = dry_biomass_wt / fresh_biomass_wt) %>% 
  group_by(code, subplot) %>% 
  summarize(m_factor = mean(m_factor, na.rm = TRUE))
...
in_field_query <- full_join(
  ...
  mutate(
    dry_wt_est_g_m2 = m_factor * (fresh_wt_a + fresh_wt_b) / 2
    )
...
summarized_biomass <- biomass_with_units_query %>% 
  arrange(year, state) %>% 
  group_by(year, state) %>% 
  summarise_at(
    vars(matches("m2$")), 
